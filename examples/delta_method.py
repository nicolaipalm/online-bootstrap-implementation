...
print(f'True asy. variance of empirical mean ({true_mean}): {true_variance}')
print(
    f'True asy. variance of transformed empirical mean ({phi(true_mean)}): {true_variance * derivative_phi(true_mean) ** 2}')

# Perform the discrete bootstrap
bootstrap = DiscreteBootstrap(samples=samples, number_bootstrap_samples=250, phi=phi)
...
bootstrap = WeightedBootstrap(samples=samples, weights=weights, number_bootstrap_samples=250, phi=phi)
# Print bootstrapped variance of the empirical mean along with the true variance
print(f'Bootstrapped variance: \n {bootstrap.estimated_asymptotic_variance}')
print(f'Confidence interval: {bootstrap.bootstrapped_confidence_interval(alpha=0.05)}')

weights = MovingAverageWeights(samples=samples)
bootstrap = WeightedBootstrap(samples=samples, weights=weights, phi=phi)
# Print bootstrapped variance of the empirical mean along with the true variance
print(f'Bootstrapped variance: \n {bootstrap.estimated_asymptotic_variance}')
print(f'Confidence interval: {bootstrap.bootstrapped_confidence_interval(alpha=0.05)}')
